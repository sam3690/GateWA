...
export class InfraController {
  ...
  saveConfig(@Body() config: SaveConfigDto): { message: string; saved: boolean; envPath: string; profiles: string[] } {
    try {
      ...
      // Header
      envLines.push('# GateWA Configuration');
      ...
      if (config.database) {
        ...
        if (config.database.type === 'postgres') {
          if (config.database.builtIn) {
            ...
            envLines.push('DATABASE_USERNAME=gatewa');
            envLines.push('DATABASE_PASSWORD=gatewa');
            envLines.push('DATABASE_NAME=gatewa');
            ...
            envLines.push(`DATABASE_NAME=${config.database.database || 'gatewa'}`);
          }
      ...
      if (config.storage) {
        ...
        if (config.storage.type === 'local') {
          ...
          if (config.storage.builtIn) {
            ...
            envLines.push('S3_BUCKET=gatewa');
