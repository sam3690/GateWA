...
export class WebhookService {
  ...
  async test(sessionId: string, webhookId: string): Promise<{ success: boolean; statusCode?: number; error?: string }> {
    ...
    const testPayload: WebhookPayload = {
      ...
      data: {
        message: 'This is a test webhook from GateWA',
    ...
    const headers: Record<string, string> = {
      ...
      'User-Agent': 'GateWA-Webhook/1.0.0',
      'X-GateWA-Event': 'test',
      'X-GateWA-Idempotency-Key': testPayload.idempotencyKey,
      'X-GateWA-Delivery-Id': testPayload.deliveryId,
      'X-GateWA-Retry-Count': '0',
      ...webhook.headers,
    };

    if (webhook.secret) {
      headers['X-GateWA-Signature'] = this.generateSignature(body, webhook.secret);
    }
  ...
  async dispatch(sessionId: string, event: string, data: Record<string, unknown>): Promise<void> {
    ...
    for (const webhook of matchingWebhooks) {
      ...
      const headers: Record<string, string> = {
        'Content-Type': 'application/json',
        'User-Agent': 'GateWA-Webhook/1.0.0',
        'X-GateWA-Event': event,
        'X-GateWA-Idempotency-Key': idempotencyKey,
        'X-GateWA-Delivery-Id': deliveryId,
        'X-GateWA-Retry-Count': '0',
        ...webhook.headers,
      };
      ...
      if (this.queueEnabled && this.webhookQueue) {
        ...
        if (webhook.secret) {
          headers['X-GateWA-Signature'] = signature;
        }
  ...
  private async deliverWebhook(
    ...
    headers['X-GateWA-Retry-Count'] = String(attempt - 1);

    // Add signature if secret is configured and not already present
    if (webhook.secret && !headers['X-GateWA-Signature']) {
      headers['X-GateWA-Signature'] = this.generateSignature(body, webhook.secret);
    }
