...
export class DockerService implements OnModuleInit {
  ...
  /**
   * List all GateWA-related containers
   */
  async listContainers(): Promise<ContainerInfo[]> {
    ...
    try {
      ...
      return containers
        .filter(c => {
          // Filter by GateWA labels or name prefix
          const labels = c.Labels || {};
          return labels['com.gatewa.service'] || c.Names?.some(n => n.startsWith('/gatewa-'));
        })
        .map(c => ({
  ...
  async getContainerByService(service: string): Promise<Docker.Container | null> {
    ...
    try {
      const containers = await this.docker.listContainers({
        all: true,
        filters: {
          label: [`com.gatewa.service=${service}`],
        },
      });
      ...
      const match = allContainers.find(c => c.Names?.some(n => n.includes(`gatewa-${service}`) || n.includes(service)));
  ...
  private getContainerSpec(profile: string): {
    ...
    const specs: Record<string, ReturnType<typeof this.getContainerSpec>> = {
      redis: {
        ...
        volumes: [{ name: 'gatewa_redis-data', path: '/data' }],
        healthcheck: {
        ...
        labels: {
          'com.gatewa.service': 'cache',
          'com.gatewa.builtin': 'true',
        },
      },
      postgres: {
        image: 'postgres:16-alpine',
        name: 'gatewa-postgres',
        alias: 'postgres',
        // Use hardcoded defaults for built-in container (don't inherit SQLite paths)
        env: ['POSTGRES_USER=gatewa', 'POSTGRES_PASSWORD=gatewa', 'POSTGRES_DB=gatewa'],
        volumes: [{ name: 'gatewa_postgres-data', path: '/var/lib/postgresql/data' }],
        healthcheck: {
          test: ['CMD-SHELL', 'pg_isready -U gatewa'],
          interval: 5000000000,
          timeout: 3000000000,
          retries: 5,
        },
        labels: {
          'com.gatewa.service': 'database',
          'com.gatewa.builtin': 'true',
        },
      },
      minio: {
        ...
        volumes: [{ name: 'gatewa_minio-data', path: '/data' }],
        ports: [
