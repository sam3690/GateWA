...
### Adapter
An interface implementation that provides a specific capability. In GateWA, adapters are used for:
...
### API Key
Authentication token to access the GateWA API. Sent via the `X-API-Key` header.
...
### Chrome/Chromium
Browser used by Puppeteer to run WhatsApp Web. GateWA uses headless Chromium.
...
### Dashboard
Web interface to manage GateWA without using the API directly. Built with React + shadcn/ui.

### Dead Letter Queue (DLQ)
Queue that stores messages that failed after all retry attempts. Used for debugging and manual retry.

### Docker
Containerization platform for packaging and deploying applications. GateWA is distributed as a Docker image.
...
### GHCR
GitHub Container Registry - registry for storing Docker images. The GateWA image is available at `ghcr.io/sam3690/gatewa`.
...
### Multi-session
Ability to run multiple WhatsApp sessions within a single GateWA instance.

## N

### NestJS
Node.js framework for building server-side applications. The GateWA backend is built with NestJS.

### Node.js
JavaScript runtime used to run GateWA. Supported version: Node.js 20 LTS.

## O

### ORM (Object-Relational Mapping)
Library that maps objects in code to database tables. GateWA uses TypeORM.
...
### Plugin
Extension that can be added to GateWA to add functionality without modifying the core codebase.
...
### REST API
Architectural style for APIs used by GateWA. Uses HTTP methods (GET, POST, PUT, DELETE).
...
AWS object storage service. GateWA supports S3-compatible storage for media files.
...
### Swagger
API documentation tool. GateWA provides Swagger UI at `/api/docs`.
...
### TypeScript
Typed superset of JavaScript used for GateWA development.
...
### Volume (Docker)
Persistent storage for Docker containers. GateWA data is stored in volumes.

## W

### WAHA
WhatsApp HTTP API - a similar project that inspired GateWA. GateWA is built as an open-source alternative.
...
### whatsapp-web.js
Primary Node.js library used by GateWA to interact with WhatsApp Web. Uses Puppeteer to control the browser.
