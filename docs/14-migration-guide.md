...
This document provides a comprehensive guide for migrating GateWA, including:
...
GateWA v0.2+ includes built-in migration API endpoints that leverage the **Dual-Database Architecture**:
...
GateWA v0.2+ supports migrating media files between storage backends:
...
Redis in GateWA is used **only for caching** with TTL-based expiration. Cache data is ephemeral and automatically regenerates from the database.
...
const config: MigrationConfig = {
  sqlitePath: process.env.SQLITE_PATH || './data/gatewa.db',
  postgresUrl: process.env.DATABASE_URL || 'postgresql://user:pass@localhost:5432/gatewa',
...
docker exec gatewa-db pg_dump -U postgres gatewa > backup.sql
...
export DATABASE_URL=postgresql://user:pass@localhost:5432/gatewa
...
ssh old-server "sqlite3 /data/gatewa.db \
    \"SELECT * FROM sessions WHERE id='${SESSION_ID}'\" \
    -csv" > session_record.csv

# Import to new database
echo "📥 Importing session record..."
ssh new-server "sqlite3 /data/gatewa.db \
...
echo "🚀 Upgrading GateWA v0.1.x → v0.2.x"
...
docker run --rm \
  -v $(pwd)/data:/app/data \
  -e DATABASE_URL=sqlite:///app/data/gatewa.db \
  ghcr.io/sam3690/gatewa:0.2.0 \
  npm run migration:run

# 4. Migrate configuration
echo "⚙️ Migrating configuration..."
cat > .env.new << 'EOF'
# GateWA v0.2.x Configuration

# Database (unchanged if using SQLite)
DATABASE_ADAPTER=sqlite
DATABASE_URL=sqlite:./data/gatewa.db
...
docker exec gatewa npm run cli -- create-api-key --name "migrated-key"
...
echo "🚀 Upgrading GateWA v0.2.x → v1.0.0"

# Pre-flight checks
CURRENT_VERSION=$(docker inspect ghcr.io/sam3690/gatewa --format '{{.Config.Labels.version}}' 2>/dev/null || echo "unknown")
    ...
    - name: Configure staging webhooks
      command: |
        npm run cli -- set-webhook \
          --url https://staging-webhook.example.com/gatewa \
...
sqlite3 ./data/gatewa.db "PRAGMA integrity_check;"
