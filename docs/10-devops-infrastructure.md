...
RUN groupadd -r gatewa && useradd -r -g gatewa gatewa
RUN chown -R gatewa:gatewa /app
USER gatewa
...
services:
  app:
    ...
    environment:
      ...
      - DATABASE_URL=postgresql://gatewa:gatewa@postgres:5432/gatewa
...
services:
  app:
    image: ghcr.io/sam3690/gatewa:latest
    deploy:
      replicas: 1
      resources:
        limits:
...
# DATABASE_URL=postgresql://user:pass@localhost:5432/gatewa
    ...
    rules:
      ...
      - alert: ServiceDown
        ...
        labels:
          severity: critical
        annotations:
          summary: "GateWA service is down"
          description: "The GateWA application is not responding"
```
...
receivers:
  - name: 'slack-notifications'
    slack_configs:
      - channel: '#gatewa-alerts'
        send_resolved: true

  - name: 'slack-critical'
    slack_configs:
      - channel: '#gatewa-critical'
        send_resolved: true
...
```json
// monitoring/grafana/dashboards/gatewa.json
{
  "title": "GateWA Dashboard",
  "uid": "gatewa-main",
  "panels": [
    {
...
# Download from S3
aws s3 cp s3://gatewa-backups/database/db_$BACKUP_DATE.dump.gz /tmp/
aws s3 cp s3://gatewa-backups/sessions/sessions_$BACKUP_DATE.tar.gz /tmp/
