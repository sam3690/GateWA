...
This guide explains how to deploy GateWA in a horizontally scaled environment for high availability and increased capacity.
...
services:
  gatewa:
    image: ghcr.io/sam3690/gatewa:0.2.0
    deploy:
      replicas: 3
      update_config:
        parallelism: 1
    ...
    environment:
      ...
      - DATABASE_NAME=gatewa
      - DATABASE_USER=gatewa
...
# Deploy stack
docker stack deploy -c docker-compose.swarm.yml gatewa

# Scale up/down
docker service scale gatewa_gatewa=5

# Check status
docker service ls
docker service ps gatewa_gatewa
```
...
metadata:
  ...
  annotations:
    ...
    nginx.ingress.kubernetes.io/session-cookie-name: 'gatewa-session'
    nginx.ingress.kubernetes.io/session-cookie-max-age: '172800'
spec:
  ingressClassName: nginx
  rules:
    - host: gatewa.example.com
      http:
        paths:
          - path: /
            pathType: Prefix
...
# Check pods
kubectl get pods -n gatewa

# Check logs
kubectl logs -f deployment/gatewa -n gatewa

# Scale
kubectl scale statefulset gatewa --replicas=5 -n gatewa
```
      ...
      loadBalancer:
        sticky:
          cookie:
            ...
            httpOnly: true
        servers:
          - url: 'http://gatewa-1:2785'
          - url: 'http://gatewa-2:2785'
          - url: 'http://gatewa-3:2785'
        healthCheck:
          path: /api/health
          interval: 10s
          timeout: 3s
...
upstream gatewa {
    ...
    server gatewa-1:2785 weight=1 max_fails=3 fail_timeout=30s;
    server gatewa-2:2785 weight=1 max_fails=3 fail_timeout=30s;
    server gatewa-3:2785 weight=1 max_fails=3 fail_timeout=30s;
}

server {
    listen 80;
    server_name gatewa.example.com;

    location / {
        proxy_pass http://gatewa;
...
```yaml
# prometheus/gatewa-rules.yaml
groups:
  - name: gatewa
    rules:
      - alert: HighMemoryUsage
        expr: container_memory_usage_bytes{container="gatewa"} > 1.8e9
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: 'GateWA node high memory usage'

      - alert: NodeDown
        expr: up{job="gatewa"} == 0
        for: 1m
        labels:
          severity: critical
        annotations:
          summary: 'GateWA node is down'
```
