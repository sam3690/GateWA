...
docker compose logs gatewa 2>&1 | grep -i "whatsapp\|puppeteer\|browser"
...
sqlite3 ./data/gatewa.db ".timeout 30000"

# Increase timeout in configuration
DATABASE_SQLITE_BUSY_TIMEOUT=30000

# Check WAL mode
sqlite3 ./data/gatewa.db "PRAGMA journal_mode;"
# Should return: wal

# Enable WAL mode
sqlite3 ./data/gatewa.db "PRAGMA journal_mode=WAL;"
```
...
docker exec gatewa ping postgres
docker exec gatewa curl http://host.docker.internal:8080
```
...
**Q: Is GateWA safe to use?**
> A: GateWA uses unofficial WhatsApp Web API. While we implement best practices to avoid detection, there's inherent risk of account restrictions. We recommend:
...
> A: Yes, GateWA works with both personal and WhatsApp Business accounts. Note that WhatsApp Business API (official Meta API) is different and not supported.
...
> 2. Set URL: `http://gatewa:2785/api/sessions/{id}/messages`
...
if [ "$DATABASE_ADAPTER" = "postgresql" ]; then
    ...
    cp ./data/gatewa.db "$BACKUP_DIR/$DATE/"
fi
...
2. **Check logs** - `docker compose logs gatewa --tail=100`
...
- **GitHub Issues**: [github.com/sam3690/GateWA/issues](https://github.com/sam3690/GateWA/issues)
- **Discussions**: [github.com/sam3690/GateWA/discussions](https://github.com/sam3690/GateWA/discussions)
- **Discord**: [discord.gg/gatewa](https://discord.gg/gatewa) (if available)
- **Stack Overflow**: Tag with `gatewa`
---
