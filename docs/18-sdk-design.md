...
GateWA provides official SDKs for multiple programming languages to simplify API integration. This document describes the SDK design and specifications.
...
```typescript
import { GateWA } from '@gatewa/sdk';

const client = new GateWA({
...
export class GateWA extends EventEmitter {
  private config: GateWAConfig;
  ...
  constructor(config: GateWAConfig) {
    super();
...
export class GateWAError extends Error {
  constructor(
    message: string,
    public code: string,
    ...
    super(message);
    this.name = 'GateWAError';
  }
}

export class ValidationError extends GateWAError {
...
export class AuthenticationError extends GateWAError {
...
export class SessionNotFoundError extends GateWAError {
...
export class RateLimitError extends GateWAError {
...
```bash
pip install gatewa-sdk
```

### Quick Start

```python
from gatewa import GateWA
import asyncio
...
class SessionsResource:
    def __init__(self, client: GateWA):
        self.client = client
...
from .async_client import GateWA as AsyncGateWA
import asyncio

class GateWASync:
    """Synchronous wrapper for GateWA client"""

    def __init__(self, **kwargs):
        self._async_client = AsyncGateWA(**kwargs)
...
# Usage
from gatewa.sync import GateWASync

client = GateWASync(base_url="http://localhost:2785", api_key="your-key")
...
use GateWA\Resources\Sessions;
use GateWA\Resources\Messages;
use GateWA\Resources\Contacts;
use GateWA\Resources\Groups;
use GateWA\Resources\Webhooks;
use GateWA\Exceptions\GateWAException;
...
namespace GateWA\Resources;

use GateWA\Client;
use GateWA\Models\Session;
...
```typescript
// nodes/GateWA/GateWA.node.ts

import { IExecuteFunctions, INodeExecutionData, INodeType, INodeTypeDescription } from 'n8n-workflow';

export class GateWA implements INodeType {
  description: INodeTypeDescription = {
    displayName: 'GateWA',
    name: 'openWA',
    icon: 'file:gatewa.svg',
    group: ['transform'],
    version: 1,
    subtitle: '={{$parameter["operation"] + ": " + $parameter["resource"]}}',
    description: 'Send WhatsApp messages via GateWA',
    defaults: {
      name: 'GateWA',
    },
    inputs: ['main'],
...
```typescript
// nodes/GateWA/GateWATrigger.node.ts

import { IHookFunctions, IWebhookFunctions, INodeType, INodeTypeDescription, IWebhookResponseData } from 'n8n-workflow';

export class GateWATrigger implements INodeType {
  description: INodeTypeDescription = {
    displayName: 'GateWA Trigger',
    name: 'openWATrigger',
    icon: 'file:gatewa.svg',
    group: ['trigger'],
    version: 1,
    description: 'Starts workflow on GateWA events',
    defaults: {
      name: 'GateWA Trigger',
    },
    inputs: [],
  ...
  webhookMethods = {
    default: {
      async checkExists(this: IHookFunctions): Promise<boolean> {
        // Check if webhook already exists in GateWA
