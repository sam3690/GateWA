...
GateWA provides an idempotency mechanism to prevent duplicate processing on the client side.
...
| `X-GateWA-Delivery-Id` | Header carrying the delivery ID |
| `X-GateWA-Idempotency-Key` | Header carrying the idempotency key |
| `X-GateWA-Retry-Count` | Retry attempt number (0 = first attempt) |
...
async function handleWebhook(req: Request, res: Response) {
  const idempotencyKey = req.headers['x-gatewa-idempotency-key'] as string;
  const retryCount = parseInt(req.headers['x-gatewa-retry-count'] as string) || 0;
...
app.post('/webhook', (req, res) => {
  const signature = req.headers['x-gatewa-signature'];
...
In addition to the REST API, GateWA provides WebSocket for real-time updates.
...
|----------|---------|--------|
| JavaScript/TypeScript | `@gatewa/sdk` | 🔜 Planned |
| Python | `gatewa-python` | 🔜 Planned |
| PHP | `gatewa/php-sdk` | 🔜 Planned |
| Go | `gatewa-go` | 🔜 Planned |
...
# Usage
client = GateWA('http://localhost:2785/api', 'your-api-key')
