...
flowchart TB
    ...
    subgraph GateWA["GateWA Platform"]
...
GateWA is designed with a **Pluggable Architecture** that allows infrastructure components to be swapped without changing application code. This enables flexible deployments ranging from minimal single-session bots to enterprise-scale multi-tenant platforms.
...
GateWA uses NestJS Dynamic Modules for adapter injection:
...
GateWA uses the adapter pattern for infrastructure components that can be swapped per deployment needs. This allows users with limited resources to run GateWA without heavyweight external dependencies.

### Adapter Overview

```mermaid
flowchart TB
    subgraph Core["GateWA Core"]
...
GateWA supports SQLite for lightweight deployments and PostgreSQL for high-volume production.
...
export const getDatabaseConfig = (config: ConfigService): TypeOrmModuleOptions => {
  ...
  if (dbType === 'sqlite') {
    return {
      ...baseConfig,
      type: 'sqlite',
      database: config.get('database.sqlite.path', './data/gatewa.db'),
...
GateWA provides several deployment profiles for different needs:
...
DATABASE_SQLITE_PATH=./data/gatewa.db
...
DATABASE_URL=postgresql://gatewa:password@localhost:5432/gatewa
...
# Database
DATABASE_TYPE=postgres
DATABASE_URL=postgresql://gatewa:password@db-cluster:5432/gatewa
DATABASE_POOL_MAX=50

# Storage
STORAGE_TYPE=s3
STORAGE_S3_BUCKET=gatewa-media
