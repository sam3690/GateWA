...
GateWA uses a database to store:
...
GateWA supports two database backends that can be selected at deployment time:
...
GateWA v0.2+ implements a **dual-database architecture** that separates boot configuration from user data:
...
When using PostgreSQL Built-in mode, GateWA automatically:
...
GateWA provides endpoints for migrating data between database types:
...
To ensure date/time values work across both SQLite and PostgreSQL, GateWA uses a `DateTransformer` that stores dates as ISO 8601 text strings:
...
# Create backup
pg_dump -Fc $DB_NAME > $BACKUP_DIR/gatewa_$DATE.dump

# Compress
gzip $BACKUP_DIR/gatewa_$DATE.dump

# Upload to S3 (optional)
aws s3 cp $BACKUP_DIR/gatewa_$DATE.dump.gz s3://backups/gatewa/
