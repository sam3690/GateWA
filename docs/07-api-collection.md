...
This document provides a complete collection of GateWA API endpoints with request/response examples for each endpoint. It can be used as a quick reference or imported into tools such as Postman, Insomnia, or Bruno.
...
**Response:**
```
# HELP gatewa_sessions_total Total number of sessions
# TYPE gatewa_sessions_total gauge
gatewa_sessions_total{status="connected"} 4
gatewa_sessions_total{status="disconnected"} 1

# HELP gatewa_messages_total Total messages processed
# TYPE gatewa_messages_total counter
gatewa_messages_total{direction="incoming"} 15234
gatewa_messages_total{direction="outgoing"} 8567

# HELP gatewa_api_requests_total Total API requests
# TYPE gatewa_api_requests_total counter
gatewa_api_requests_total{method="GET",status="200"} 45678
gatewa_api_requests_total{method="POST",status="200"} 12345
```
...
```json
{
  "info": {
    "name": "GateWA API",
    "description": "GateWA WhatsApp API Gateway",
...
Download: [gatewa-postman-collection.json](./assets/gatewa-postman-collection.json)

## 07.10 cURL Examples Collection

```bash
#!/bin/bash
# gatewa-curl-examples.sh
