...
|-----------|--------|-------|
| **@gatewa/plugin-sdk** | 🔜 Planned | NPM package not yet published |
...
The plugin architecture enables GateWA extensibility without modifying the core codebase. Plugins can add new features, integrate with external services, or customize behavior.
...
```json
{
  ...
  "version": "1.0.0",
  "description": "An awesome plugin for GateWA",
  "author": "Your Name",
  "license": "MIT",
...
import { GateWAPlugin, PluginContext, MessageEvent, HookResult } from '@gatewa/plugin-sdk';
...
export default class MyAwesomePlugin implements GateWAPlugin<MyPluginConfig> {
  name = 'my-awesome-plugin';
  version = '1.0.0';
...
```typescript
// @gatewa/plugin-sdk/types.ts

export interface GateWAPlugin<TConfig = any> {
  name: string;
  version: string;
...
```mermaid
sequenceDiagram
    participant Core as GateWA Core
...
import { GateWAPlugin, PluginContext, PluginManifest } from './types';
...
import { GateWAPlugin, PluginContext, MessageEvent } from '@gatewa/plugin-sdk';
...
export default class AutoReplyPlugin implements GateWAPlugin<AutoReplyConfig> {
  name = 'auto-reply';
  version = '1.0.0';
...
export default class TranslationPlugin implements GateWAPlugin<TranslationConfig> {
  name = 'translation';
  version = '1.0.0';
