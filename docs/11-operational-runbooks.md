...
This document contains Standard Operating Procedures (SOP) for GateWA operations, including incident response, maintenance procedures, and troubleshooting guides.
...
docker compose logs --tail=100 gatewa
...
# A. Container crashed
docker compose logs gatewa 2>&1 | grep -i "error\|fatal\|crash"

# B. Out of memory
docker compose logs gatewa 2>&1 | grep -i "oom\|memory"

# C. Database connection
docker compose logs gatewa 2>&1 | grep -i "database\|connection refused"

# 5. Apply fix based on cause:

# A. Simple restart
docker compose restart gatewa
...
docker compose logs gatewa 2>&1 | grep -i "{sessionId}" | tail -20
...
docker compose logs gatewa 2>&1 | grep -i "heap\|memory\|gc"
...
curl -v https://your-webhook-endpoint.com/gatewa

# B. SSL certificate issues
curl -v --insecure https://your-webhook-endpoint.com/gatewa

# C. Timeout
curl -v --max-time 30 https://your-webhook-endpoint.com/gatewa

# D. Authentication failed
curl -v -H "Authorization: Bearer token" \
  https://your-webhook-endpoint.com/gatewa
...
# Verify backup
ls -la /backups/gatewa/$(date +%Y%m%d)/
...
# 2. Create backup
./scripts/backup.sh
BACKUP_DIR="/backups/gatewa/$(date +%Y%m%d-%H%M%S)"

# 3. Export current state
docker compose exec gatewa npm run export -- --output /tmp/export.json
docker cp gatewa:/tmp/export.json $BACKUP_DIR/

# 4. Stop services
docker compose down

# 5. Update version in docker-compose.yml
# Change: image: ghcr.io/sam3690/gatewa:0.1.0
# To:     image: ghcr.io/sam3690/gatewa:0.2.0

# 6. Pull new image
docker compose pull

# 7. Run database migrations (if any)
docker compose run --rm gatewa npm run migration:run
...
BACKUP_DIR="/backups/gatewa/$DATE"
RETENTION_DAYS=30
...
if [ "$DATABASE_ADAPTER" = "postgresql" ]; then
    ...
    docker compose exec -T postgres pg_dump -U postgres gatewa > "$BACKUP_DIR/database.sql"
else
    echo "Backing up SQLite..."
    # Use SQLite online backup
    docker compose exec gatewa sqlite3 /app/data/gatewa.db ".backup /tmp/backup.db"
    docker cp gatewa:/tmp/backup.db "$BACKUP_DIR/gatewa.db"
fi
...
tar -czf "$BACKUP_DIR.tar.gz" -C "/backups/gatewa" "$DATE"
rm -rf "$BACKUP_DIR"

# 6. Cleanup old backups
echo "Cleaning up old backups..."
find /backups/gatewa -name "*.tar.gz" -mtime +$RETENTION_DAYS -delete
...
if [ -f "$RESTORE_DIR/*/database.sql" ]; then
    ...
    sleep 10
    docker compose exec -T postgres psql -U postgres gatewa < "$RESTORE_DIR"/*/database.sql
elif [ -f "$RESTORE_DIR/*/gatewa.db" ]; then
    echo "Restoring SQLite..."
    cp "$RESTORE_DIR"/*/gatewa.db ./data/
fi
