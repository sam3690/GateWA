#!/bin/bash
# GateWA Smart Orchestration Script
...
cmd_start() {
    ...
    echo ""
    log_success "GateWA started successfully!"
    echo ""
...
# Show status
cmd_status() {
    log_info "GateWA container status:"
    echo ""
    cd "$PROJECT_DIR"
...
# Build images
cmd_build() {
    log_info "Building GateWA images..."
    load_env
    local profiles=$(get_profiles)
    cd "$PROJECT_DIR"
...
cmd_help() {
    echo ""
    echo "GateWA Smart Orchestration Script"
    echo ""
    echo "Usage: $0 <command> [options]"
    echo ""
    echo "Commands:"
    echo "  start       Start GateWA with auto-detected profiles"
    echo "  stop        Stop all GateWA containers"
    echo "  restart     Restart GateWA"
    echo "  status      Show container status"
    echo "  logs        Show logs (default: gatewa-api)"
